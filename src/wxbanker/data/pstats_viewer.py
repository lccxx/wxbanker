...
import os.path
import pstats #@UnresolvedImport
import sys
import re
import threading
import traceback
...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        for methodName in dir(self):
            ...
            if method.__doc__.startswith('handle:'):
                handle, path_re = method.__doc__.split(':') #@UnusedVariable
    ...
    def index(self):
        ...
        for func in self.print_list:
            file, line, func_name = func #@UnusedVariable
            primitive_calls, total_calls, exclusive_time, inclusive_time, callers = self.stats.stats[func] #@UnusedVariable
...
''' % (self.filename, formatTime(self.total_time), '\n'.join(table))
    ...
    def func(self, id):
        ...
        def sortedByInclusive(items):
            ...
            return [y for x, y in reversed(sorted(sortable))] #@UnusedVariable
...
def main(argv):
    ...
    launch_thread = startThread(lambda: launch_browser(port)) #@UnusedVariable
