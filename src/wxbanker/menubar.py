...
import wx, webbrowser, os #@UnusedImport
from wx.lib.wordwrap import wordwrap #@UnusedImport
from wx.lib.pubsub import Publisher #@UnresolvedImport

from wxbanker import version, localization, debug, fileservice, brandedframe #@UnusedImport
...
class BankMenuBar(wx.MenuBar):
    ...
    def __init__(self, bankController, *args, **kwargs):
        ...
        importCsvMenu = fileMenu.Append(self.ID_IMPORT_CSV, _("Import from CSV"), _("Import transactions from a CSV file")) #@UnusedVariable
        exportCsvMenu = fileMenu.Append(self.ID_EXPORT_CSV, _("Export to CSV"), _("Export transactions to a CSV file")) #@UnusedVariable
        fileMenu.AppendSeparator()
        quitItem = fileMenu.Append(wx.ID_EXIT) #@UnusedVariable
        ...
        for i, cstr in enumerate(sorted(self.currencyStrings[1:])): #@UnusedVariable
        ...
        ## TRANSLATORS: Put the ampersand (&) before the letter to use as the Alt shortcut.
        aboutItem = helpMenu.Append(wx.ID_ABOUT, _("&About"), _("More information about wxBanker")) #@UnusedVariable
