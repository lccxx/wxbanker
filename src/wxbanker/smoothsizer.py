...
class SmoothSizerMixin:
    ...
    def SmoothInsertOrig(self, item, prop=0, flag=0, border=0, userData=None, expandIn=(1, 1), smooth=None, index=-1, spacer=None, pixels=None):
        ...
        if spacer is None: #this is our first time here
            ...
            for i, sitem in enumerate(self.Children):
                if self.GetObject(sitem) is self.GetObject(sizerItem): #@UndefinedVariable
                    pos = i; break
    ...
    def SmoothShow(self, item, pixels=None, smooth=None, index=-1, spacer=None, callback=None, *args, **kwargs):
        ...
        if not smooth or tuple(space) > tuple(pixels): ## space > pixels
            ...
            print 0
            spacer.SetSpacer(space)
            result = wx.CallLater(self.Increment*1000, self.SmoothShow, item, pixels, smooth, index, spacer, callback, *args, **kwargs) ##X @UnusedVariable
    ...
    def SmoothHide(self, item, pixels=None, smooth=None, index=-1, spacer=None, callback=None, *args, **kwargs):
        ...
        if not smooth or tuple(space) <= (0, 0): ## space > pixels
            ...
            spacer.SetSpacer(space)
            result = wx.CallLater(self.Increment*1000, self.SmoothHide, item, pixels, smooth, index, spacer, callback, *args, **kwargs) #@UnusedVariable
...
class DemoFrame(wx.Frame):
    def __init__(self, parent=None):
        ...
        self.delayCtrl = wx.lib.intctrl.IntCtrl(panel, value=200, min=0, limited=True, size=(50, -1)) #@UndefinedVariable
        self.incrementCtrl = wx.lib.intctrl.IntCtrl(panel, value=15, min=0, limited=True, size=(50, -1)) #@UndefinedVariable
...
if __name__ == "__main__":
    app = wx.App(False)
    import wx.lib.intctrl #@UnusedImport
    frame = DemoFrame()
    app.MainLoop()
