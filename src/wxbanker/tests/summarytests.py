...
#    summarytests.py: Copyright 2007-2010 Mike Rooney <mrooney@ubuntu.com>
#
...
from wxbanker import bankobjects #@UnusedImport
import unittest, datetime
from wxbanker.tests.testbase import today, yesterday, one #@UnusedImport
...
class SummaryTests(testbase.TestCaseWithController):
    ...
    def testTransactionsBeforeRangeAreCounted(self):
        amounts, start, delta = self.get([(today-one, 3), (today, 2), (today+one, 1)], 3, None, (today, today+one)) #@UnusedVariable
        self.assertEqual(amounts, [5.0, 5.0, 6.0])
        
    def testDateRangeEndingBeforeTodayWorks(self):
        amounts, start, delta = self.get([(today-one*2, 3), (today-one, 2), (today, 1)], 2, None, (today-one*2, today-one)) #@UnusedVariable
